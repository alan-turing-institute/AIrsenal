...
def run_optimization(
    ...
    with console.status("Optimising transfers..."):
        ...
        with progress_bar(transient=True) as progress:
            ...
            # worker cannot exit until its queue feeder threads have flushed,
            # and stopping the consumer first leaves nothing draining
            # progress_queue while p.join() waits: a worker with a full pipe can
            # then never finish writing, and the join waits for ever. Joining
            # first costs nothing and removes the window.
            for _ in procs:
                squeue.put(None)
            for p in procs:
                p.join()

            # No worker is left to write to either queue, and everything they
            # wrote has been flushed, so these sentinels cannot overtake real
            # messages.
