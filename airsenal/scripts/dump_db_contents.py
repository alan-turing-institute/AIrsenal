...
def main():
    ...
    with open(output_path, 'w') as csvfile:
        ...
        for player in session.query(Player).all():
            ...
            row = {}
            for field in player:
                if isinstance(player[field], str) or isinstance(player[field], int) \
    ...
    player_attributes_fieldnames = [
        'id', 'player_id', 'season', 'gw_valid_from', 'current_price', 'team', 'position']
    output_path = os.path.join(os.path.dirname(__file__),
                               "../data/player_attributes.csv")
    with open(output_path, 'w') as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=player_attributes_fieldnames)
        writer.writeheader()
        for player_attributes in session.query(PlayerAttributes).all():
            ...
            for field in player_attributes:
                if isinstance(player_attributes[field], str) or isinstance(player_attributes[field], int) or \
                        isinstance(player_attributes[field], float):
                    row[field] = player_attributes[field]
            writer.writerow(row)
    print(" ==== dumped PlayerAttributes database === ")

    # Dump Fixture database
    fixture_fieldnames = ['fixture_id', 'date', 'gameweek',
                          'home_team', 'away_team', 'season', 'tag', 'player_id']
    output_path = os.path.join(os.path.dirname(__file__),
                               "../data/fixtures.csv")
    with open(output_path, 'w') as csvfile:
    ...
    result_fieldnames = ['result_id', 'fixture_id',
    ...
    with open(output_path, 'w') as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=fifa_team_rating_fieldnames)
        writer.writeheader()
        for fifa_team_rating in session.query(FifaTeamRating).all():
            ...
            for field in fifa_team_rating:
                if isinstance(fifa_team_rating[field], str) or isinstance(fifa_team_rating[field], int) or \
                        isinstance(fifa_team_rating[field], float):
                    row[field] = fifa_team_rating[field]
            writer.writerow(row)
    print(" ==== dumped FifaTeamRating database === ")

    # Dump Transaction database
    transaction_fieldnames = ['id', 'player_id', 'gameweek',
                              'bought_or_sold', 'season', 'tag', 'price']
    output_path = os.path.join(os.path.dirname(__file__),
    ...
    with open(output_path, 'w') as csvfile:
        ...
        for transaction in session.query(Transaction).all():
            ...
            for field in transaction:
                if isinstance(transaction[field], str) or isinstance(transaction[field], int) or \
                        isinstance(transaction[field], float):
                    row[field] = transaction[field]
            writer.writerow(row)
    print(" ==== dumped Transaction database === ")

    # Dump PlayerScore database
    player_score_fieldnames = ['id', 'player_team', 'opponent', 'points', 'goals',
                               'assists', 'bonus', 'conceded', 'minutes', 'player_id',
                               'result_id', 'fixture_id', 'clean_sheets', 'own_goals',
                               'penalties_saved', 'penalties_missed', 'yellow_cards', 'red_cards',
                               'saves', 'bps', 'influence', 'creativity', 'threat',
                               'ict_index', 'value', 'transfers_balance', 'selected',
    ...
    with open(output_path, 'w') as csvfile:
        ...
        for player_score in session.query(PlayerScore).all():
            ...
            for field in player_score:
                if isinstance(player_score[field], str) or isinstance(player_score[field], int) or \
                        isinstance(player_score[field], float):
