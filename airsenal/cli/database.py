...
@click.option("--setup", is_flag=True,
              help="Set up the initial database for `airsenal`.")
@click.option("--update", is_flag=True,
              help="Update the database with the most recent data.")
@click.option("--fpl-team-id", type=int, required=False, default=None,
              help="FPL team ID")
@click.option("--dump", is_flag=True,
              help="Dump database to CSV files in `airsenal/data`.")
@click.option("--rebuild", is_flag=True,
              help="If set, deletes and recreates any pre-existing AIrsenal database.")
@click.option("--noattr", is_flag=True,
              help="Set if you don't want to update player attributes.")
