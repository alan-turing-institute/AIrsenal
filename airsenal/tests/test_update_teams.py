...
def test_fills_teams_from_the_api():
...
def test_empty_api_response_is_an_error():
    """Silently filling nothing would leave every fixture lookup failing later,
    a long way from the cause."""
    with session_scope() as ts, pytest.raises(RuntimeError, match="No teams"):
        fill_team_table_from_api("2829", dbsession=ts, apifetcher=FakeFetcher([]))


def test_update_teams_leaves_a_populated_season_alone():
    with session_scope() as ts:
        team = Team()
        team.name = "ARS"
