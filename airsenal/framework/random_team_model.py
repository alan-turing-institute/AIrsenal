...
class RandomMatchPredictor(BaseMatchPredictor):
    ...
    def fit(self, training_data: Dict[str, Union[Iterable[str], Iterable[float]]],
        ...
        self.corr_coef = np.random.randn(self.num_samples, )
        self.rho = np.random.randn(self.num_samples, )
        return self
    ...
    def predict_score_proba(self,
        ...
        sampled_probs = np.random.randn(self.num_samples, len(home_team)) * home_probs * away_probs
    ...
    def add_new_team(self, team_name: str, team_covariates: Optional[np.array] = None):
        ...
        attack = np.random.randn(self.num_samples,)
        defence = np.random.randn(self.num_samples,)
        home_advantage = np.random.randn(self.num_samples,)
