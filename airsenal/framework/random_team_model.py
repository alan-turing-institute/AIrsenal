...
class RandomMatchPredictor(BaseMatchPredictor):
    ...
    def fit(self, training_data: Dict[str, Union[Iterable[str], Iterable[float]]],
        random_state: int = 42):
        ...
        self.attack = np.random.randn(1000, len(self.teams))
        self.defence = np.random.randn(1000, len(self.teams))
        self.home_advantage = np.random.randn(1000, len(self.teams))
        self.corr_coef = np.random.randn(1000, )
        self.rho = np.random.randn(1000, )
        return self

    def predict_score_proba(self,
        ...
        home_probs = np.random.randn(1000, len(home_team))
        away_probs = np.random.randn(1000, len(away_team))

        sampled_probs = np.random.randn(1000, len(home_team)) * home_probs * away_probs
    ...
    def add_new_team(self, team_name: str, team_covariates: Optional[np.array] = None):
        ...
        attack = np.random.randn(1000,)
        defence = np.random.randn(1000,)
        home_advantage = np.random.randn(1000,)
