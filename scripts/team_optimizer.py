...
def make_optimimum_substitution(team,method="AIv1",gameweek_range=None):
    ...
    for pos in ["GK","DEF","MID","FWD"]:
        ordered_player_lists[pos] = get_predicted_points(gameweek=gameweek_range,
                                                         position=pos,
                                                         method=method)
...
def apply_strategy(strat, starting_team, method="AIv1",baseline_dict=None):
    ...
    strategy_output = {"total_score" : -1*strat[1],  # points hit from this strategy
                       "points_per_gw": {},
                       "players_in": {},
                       "players_out": {}
                       }
    for gw in sorted(strat[0].keys()): # make sure we go through gameweeks in order
        if strat[0][gw] == 0:  # no transfers that gameweek
            ...
            new_team, rp, ap = make_random_substitutions(new_team,
                                                         strat[0][gw],
                                                         method,
                                                         gw)
            score = new_team.get_expected_points(gw)
        ...
        if baseline_dict and baseline_dict[gw] - \
           strategy_output["total_score"] > 5:
...
if __name__ == "__main__":
    ...
    print("Baseline score for next {} gw:  {}".format(num_weeks_ahead,
                                                      baseline))
    ...
    for iteration in range(num_iterations):
        t = get_starting_team()
        for s in strategies:
            strategy_output = apply_strategy(strat=s,
                                             starting_team=t,
                                             method='AIv1',
