...
def get_defending_points(player_id, position,
    ...
    if position == "DEF" or position == "GK":
        ...
        for n in range(7):
            defending_points -= n // 2  * expected_minutes/90 *\
                                model_team.concede_n_probability(n,team,
                                                                 opponent,
                                                                 is_home)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--weeks_ahead",help="how many weeks ahead to fill",type=int,
                        default=5)
    parser.add_argument("--method",help="name or version to identify predictions",
                        default="AIv1")
    ...
    for pos in ["GK","DEF","MID","FWD"]:
        for player in list_players(position=pos):
            all_predictions[player] = get_predicted_points(player,
                                                           model_team,
                                                           df_player,
                                                           args.weeks_ahead)
