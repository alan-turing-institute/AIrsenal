...
def find_players_for_team(team, summary_data):
    ...
    for player in summary_data:
        for k, v in team_name_json.items():
            if player["team"] in v and (team == k or team in v):
    ...
    for player in player_list:
        ...
        for match in player_detail:
            ...
            for tk, tv in team_name_json.items():
                if match["opponent"] in tv and (team == tk or team in tv):
...
if __name__ == "__main__":
    ...
    detail_data = json.load(open("../data/player_details_{}.json".format(season)))
    ...
    for line in results_file.readlines():
        ...
        home_players = find_players_for_team(home_team, summary_data)
        gameweeks = find_matches_against_team(away_team, home_players, detail_data)
        ...
        if len(gameweeks) == 2:
            ...
            print(
                "Some sort of problem, only saw {} gameweeks for {} vs {}".format(
                    len(gameweeks), home_team, away_team
                )
            )
