...
## What the moved seams were for

Both models AIrsenal now predicts with were impossible before phase 2:
`XGTeamModel` predicts a mean and no distribution over goal counts, and
`XGPlayerModel` reaches a share of a team's goals without a posterior over them.
What each of their numbers was measured to be, and which ideas were tried and
rejected, is in [xg-models.md](xg-models.md).
